...
| Send or read Teams messages                          | Teams (`/add-teams`)                  |
...
| Connect to any other service                         | Generic (`/add-connector`)            |

If no connector exists for the required functionality, say so plainly and do NOT implement a direct API call as a workaround — it will not work in production.
